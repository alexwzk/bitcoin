...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("tickethash", block.hashTicket.GetHex()));
    result.push_back(Pair("rewardsighash", block.hashRewardSig.GetHex()));
    Array txs;
    ...
    //TODO block ticket and reward signaue
    if (blockindex->pprev)
