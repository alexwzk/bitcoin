...
uint256 CBlock::CheckMerkleBranch(uint256 hash,
		const std::vector<uint256>& vMerkleBranch, int nIndex) {
	if (nIndex == -1)
		return 0;
...
std::string CBlock::ToString() const {
	std::stringstream s;
	s << strprintf("CBlock(hash=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, hashTicket=%s, hashReward=%s, nTime=%u, nBits=%08x, nNonce=%u, vtx=%u)\n",
	GetHash().ToString(),
	nVersion,
	...
	/*if (ticket.mkproofs.size() > 0) {
		...
		for (unsigned int i = 0;
				i < ticket.mkproofs[0].returnHashSiblings().size(); i++)
	...
	/*if (ticket.signatures.size() > 0) {
		...
		s << "mk proof: (";
		for (unsigned int i = 0;
				i < ticket.signatures[0].returnHashSiblings().size(); i++)
	...
	/*if (vsignreward.size() > 0) {
		...
		s << "mk proof: (";
		for (unsigned int i = 0; i < vsignreward[0].returnHashSiblings().size();
				i++)
